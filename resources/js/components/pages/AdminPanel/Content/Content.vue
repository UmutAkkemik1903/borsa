    ...
    <div class="pcoded-inner-content">
        <div class="main-body">
            <div class="page-wrapper">
                <div class="page-body">
                    <div><content-modal :item="item" v-on:onSaved="refreshData" ></content-modal></div>
                    <div class="card">
                        <div class="card-header">
                            <h5>Makale</h5>
                            <span>Makalenizi buradan tanımlayabilirsiniz!</span>
                                    ...
                                    <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
                                        <div class="input-group">
                                            <div class="active-cyan-3 active-cyan-4">
                                                <input  type="text" v-model="search_input" v-on:input="contentSearch()" placeholder="Makale Ara" aria-label="Makale Ara" aria-describedby="button-addon1">
                                            </div>
                                        </div>
                                    </form>
                                </div>
...
export default {
    name: "Content",
    components:{ContentModal,Paginate},
    data(){
        return {
            meta:{},
            lastPage:"",
    ...
    methods: {
        ...
        fetchData(page = 1) {
            axios.get('content', {params: {page}}).then(response => {
            ...
            swal.fire({
                ...
                if (result.value) {
                    axios.delete("/content/" + id)
                        .then(response => {
                            this.fetchData();
                            toastr.warning('Kayıt Silindi', 'Makale')
                        })
                        .catch(error => {
                            if (error.response != null)
                                this.errorMessage = error.response.data.message;
        ...
        contentSearch(){
            if (this.search_input.length > 1){
                this.content = {};
                axios.get('/contentSearch/'+this.search_input).then(response => {
                    this.content = response.data.data;
                    this.meta = response.data;
                    this.lastPage = this.meta.last_page;
                })
